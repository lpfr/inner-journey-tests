...
import InnerHouseSceneEffects from "./components/effects/InnerHouseSceneEffects.jsx";
...
const PRELOAD_IMAGES = [
  ...
  "/scenes/mountain-lighthouse/lighthouse_home.png",
  "/scenes/mountain-lighthouse/lighthouse_intro.png",
  "/scenes/mountain-lighthouse/wet_map_path.png",
  "/scenes/mountain-lighthouse/lighthouse_door.png",
...
function App() {
    ...
    if (withSound) {
      if (activeScene?.id === "fog-museum") {
        ...
        if (activeScene?.audio?.ambience) {
          enableSound(false);
          stopAll();
          const ambienceVolume = ["sinking-city", "glass-greenhouse", "room-for-two", "mask-ball", "mountain-lighthouse", "inner-house"].includes(activeScene.id) ? 0.16 : 0.14;
  ...
  function chooseChoice(choice) {
    ...
    if (activeScene?.audio?.choice) {
      const choiceVolume = ["sinking-city", "glass-greenhouse", "room-for-two", "mask-ball", "mountain-lighthouse", "inner-house"].includes(activeScene.id) ? 0.3 : 0.14;
    ...
    if (activeScene?.id === "mountain-lighthouse" && activeScene?.audio?.step) {
      playSceneClip(activeScene.audio.step, false, 0.14);
    }
    ...
    window.setTimeout(() => {
      ...
      if (nextStep?.id === "result") {
        ...
        } else if (activeScene?.id === "mountain-lighthouse" && activeScene?.audio?.ending) {
          stopAll();
          playSceneClip(activeScene.audio.ending, false, 0.14);
